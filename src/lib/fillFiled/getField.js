...
let getPurePath = PublicApp.Router.getPurePath
...
function nowDate() {
  if (baseConfig.base.allowTimeCheck && baseConfig.base.logflag) {
    return (
      +new Date() +
      (storage.getLocal('ANSSERVERTIME')
        ? Number(storage.getLocal('ANSSERVERTIME'))
        : 0)
    )
  } else {
    return +new Date()
...
function timeCalibration() {
  if (
    storage.getLocal('ANSSERVERTIME') &&
    baseConfig.base.allowTimeCheck &&
    baseConfig.base.logflag
  ) {
    return true
  }
  return false
...
function getOs() {
  return baseConfig.system.system.system
    ? baseConfig.system.system.system.split(' ')[0]
    : ''
}
...
function getOsVersion() {
  return baseConfig.system.system.system
    ? baseConfig.system.system.system.split(' ')[1]
    : ''
}
...
function valueToString(value) {
  if (Util.paramType(value) === 'Object') {
    ...
    Util.paramType(value) === 'Undefined' ||
    Util.paramType(value) === 'Null'
  ) {
    return ''
  } else {
...
function getEleContent() {
  ...
  if (userObj.currentTarget) {
    return (
      userObj.currentTarget.dataset &&
      valueToString(userObj.currentTarget.dataset.content)
    )
  } else {
...
function getEleType() {
  let userObj = baseConfig.base.userObj
  return (
    userObj.currentTarget &&
    userObj.currentTarget.dataset &&
    valueToString(userObj.currentTarget.dataset.type)
  )
}

function getEleFunction() {
  let userObj = baseConfig.base.userObj
  const datasetFuncName =
    userObj.currentTarget &&
    userObj.currentTarget.dataset &&
    valueToString(userObj.currentTarget.dataset.function)
  return userObj.element_function || datasetFuncName
}
...
function getEleName() {
  ...
  return (
    userObj.currentTarget &&
    userObj.currentTarget.dataset &&
    valueToString(userObj.currentTarget.dataset.name)
  )
}
