import { appStart } from './template/appStart';
...
import { checkTimePromise } from '../ProgramDiff/Common/checkTime/checkTimePromise'
let customFlag = true
let startUpFnCache = []
function initFn (callback, isS, superFlag) {
    return function () {
        if (customFlag == true && (baseConfig.system.system == null || baseConfig.system.netWork == null)) {
            customFlag == false;
            Promise.all([systemPromise(), netWorkPromise(), checkTimePromise()]).then((res) => {
                customFlag = true
                ...
                if (isS || baseConfig.isStartUp == true) {
...
class API {
    constructor() {
        this.appStart = initFn(appStart, true, false);
